...
export const createSmartAccountForEmail = async (wallet: ConnectedWallet): Promise<BiconomySmartAccountV2> => {
    ...
    _signTypedData: async (
      ...
      if (provider && provider.request) {
        return await provider.request({
          ...
          params: [wallet.address, JSON.stringify({ domain, types, message: value })]
        }) as string;
      }
      throw new Error('Provider not available');
    }
...
export const sendSponsoredTransaction = async (
  ...
  try {
    ...
    if (paymasterAndDataResponse) {
      // Using type assertion since we know this is valid for Biconomy
      (userOp as any).paymasterAndData = paymasterAndDataResponse;
    }
